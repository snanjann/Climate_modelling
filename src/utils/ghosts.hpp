...
inline GhostCells1D capture_ghost_cells(const Field1D& U) {
    return {U.u.front(), U.u.back()};
}

inline GhostCells2D capture_ghost_cells(const Field2D& U) {
    const auto& G = *U.g;
    GhostCells2D ghosts;
...
inline GhostCells3D capture_ghost_cells(const Field3D& U) {
    const auto& G = *U.g;
    GhostCells3D ghosts;
