// Standard C++ libraries for timing, math functions, I/O, containers, file handling, etc.
#include <chrono>
#include <cmath>
#include <functional>
#include <iostream>
...
// Project headers: shared helpers, grid definitions, operators, and solvers
...
// Stores general information about a simulation run
struct RunMeta {
    std::string mode;
    int threads;
    int Nx, Ny, Nz;
...
// Holds summary numbers for 2D results
struct Stats2D {
...
// Holds summary numbers for 3D results
struct Stats3D {
    double min, max, mean, mass, l2;
    double peak_value;
...
// Computes basic statistics for a 2D field
...
// Computes basic statistics for a 3D field
...
// Ensures the log file has a header line (creates it if missing)
...
// Appends one 2D run to the log file
...
// Appends one 3D run to the log file
...
// Writes a horizontal slice through the middle of a 2D field
...
// Writes a center slice through the middle of a 3D field
...
// Runs a 2D heat-spreading test using the ADI solver
...
void run_advection_2d() {
    ...
    Laplace2D L(G);
    Advection2D A(
        [](double, double, double) { return 0.5; },  // motion in x
        [](double, double, double) { return 0.0; }   // no motion in y
    );
...
// Runs 2D heat-spreading with a moving heat source
void run_forcing_2d() {
    auto wall_start = std::chrono::high_resolution_clock::now();
...
// Runs the full 3D model: spreading + flow + moving heat source
void run_full_climate_3d() {
    ...
    // Moving hot spot inside the cube
    ...
    // Initial wave-shaped temperature field
    for (int k = 0; k <= G.Nz; ++k)
        for (int j = 0; j <= G.Ny; ++j)
            for (int i = 0; i <= G.Nx; ++i)
                U.u[U.id(i, j, k)]
                    = std::sin(M_PI * G.x[i]) * std::sin(M_PI * G.y[j]) * std::sin(M_PI * G.z[k]);
    bc.apply(U);

    // Choose safe time step for motion and spreading
    ...
    int jmid = G.Ny / 2, kmid = G.Nz / 2;
    std::cout << "Centerline value (x in [0,1], y=0.5,z=0.5): "
              << U.u[U.id(G.Nx / 2, jmid, kmid)] << std::endl;
...
// Main entry point: chooses which simulation to run
int main(int argc, char** argv) {
    // Make sure the user gives a mode
    ...
    fs::create_directories(RESULTS_DIR);  // Ensure output folder exists
